...
describe('defibrillators CRUD part reducer', () => {
	...
	it(`should handle ${types.CREATE_DEF_POINT} action`, () => {
		const expectedState = {...initialState, data: [...initialState.data, mockNewDefInfo]};

		expect(defReducer(initialState, {
			type: types.CREATE_DEF_POINT,
			newDef: mockNewDefInfo
		})).toEqual(expectedState);
	});
	it(`should have length 1 when ${types.CREATE_DEF_POINT} action have`, () => {
		const expectedState = {...initialState, data: [...initialState.data, mockNewDefInfo]};
	...
	it(`should handle ${types.DELETE_DEF_POINT} action`, () => {
		...
		const expectedState = {...initialState, data: initialState.data.filter(def => def.id !== id)};
	...
	it(`should handle ${types.EDIT_DEF_POINT}`, () => {
		...
		expect(defReducer({...initialState, data: mockData}, {
